...
export const whyDataWarehouseContent: LessonContent = {
  ...
  sections: [
    ...
    },
    {
      title: '交易和分析是不同负载',
      paragraphs: [
        '在线交易更在意单笔写入是否快速、准确，分析则经常需要扫描大量记录、跨系统关联并观察一段时间的变化。如果把所有分析查询直接压到核心交易库上，不仅容易拖慢正常下单，还可能因大范围扫表导致数据库 CPU 飙升甚至锁表。',
        '数据仓库提供一个缓冲和组织层，接入来源数据后按分析需要统一口径、保留历史并生成可复用的数据集。',
      ],
    },
  ],
  visualization: {
