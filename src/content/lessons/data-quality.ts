...
export const dataQualityContent: LessonContent = {
  ...
  opening: {
    eyebrow: '调度全部显示打勾，存款余额就一定准吗？',
    title: '任务成功了，数据就一定准吗？',
    intro:
      '日批按时结束，Scheduler 页面也全部显示 success，但余额指标可能少一行、重复一笔或和明细对不上。打开质量检查，逐条找出完整性、唯一性、编码、引用、对账和迟到分区的问题。',
    cards: [
    ...
    question: '如果任务是绿色的，但存款余额少了一笔，你会先查哪一条规则？',
  },
  subtitle:
    '调度成功后，逐条核对完整性、唯一性、币种编码、引用关系、余额对账和数据时效，再决定指标能不能发布。',
  quickSummary:
  ...
  sections: [
    {
      ...
      paragraphs: [
        '调度页面的 success 只能证明任务没有报错，不能证明每一条账户余额都已到达、金额没有重复或汇总没有漂移。质量检查要把运行结果和具体数据样本放在一起看。',
        '默认场景故意从 DWD 输出拿掉 A002。DWD task 仍然是 success，但完整性规则会指出少了哪一条余额，DWD / DWS 对账也会暴露金额差异。',
      ],
      bullets: [
    ...
    },
    {
      ...
      title: '六种质量问题，六种调查入口',
      paragraphs: [
        '完整性和唯一性要回到 DWD 账户日粒度；有效性要回到币种枚举；引用完整性要沿账户关系确认；跨表对账要比较 DWD 目标余额和 DWS 汇总；Freshness 则要把业务日期和 Scheduler 的实际完成时间放在一起看。',
        '例如，缺少 A002 会同时影响 DWD 完整性和 DWS 对账。把异常落到具体行、字段和分区，才能判断应该阻断哪一个下游结果。',
      ],
      bullets: [
        '表级行数缺失：发现“账户余额少了一行”。',
        '主键重复：发现重复写入会放大余额。',
        '枚举和引用：发现币种不可解释或余额找不到账户关系。',
    ...
    },
    {
      kind: 'pitfall',
      title: '质量通过也有边界',
      text: '规则没有覆盖的字段、过宽或过窄的阈值、以及本身错误的业务口径，都可能让所有检查 pass 但指标仍然不可信。质量闸门提升可解释性，不替代业务定义、血缘调查和治理责任。',
    },
  ],
}
