    ...
    <div className="sql-workbench__focus sql-workbench__focus--join">
      ...
      <div className="sql-workbench__join-match-grid">
        ...
        <article className="sql-workbench__join-reason">
          ...
          <p>
            {mode === 'wrong'
              ? 'A001 的 100000 被复制到三个账户介质行，整表按行求和因此从 300000 变成 500000：多出来的 200000，就是 A001 被多算的两次。SQL 没报错，但按行求和已经失去账户日余额的含义。'
              : '如果业务只需要存款余额，就不需要把账户介质接进来；若只是想判断账户有没有介质，可以先把 AccountMedium 聚合到账户级，或使用 EXISTS。'}
          </p>
        </article>
      </div>
