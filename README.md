...
| ⚖️ **湖仓四节实验台**       | 沿 Banking Teaching Domain 的 Source → Lake → Warehouse 链路，观察 Lake-first、复制、Table Layer、Snapshot 和湖仓一体的工程取舍。 |
| 📏 **存款余额指标实验台**   | 对比多个存款余额结果，补全指标定义卡，切换时点/期间时间语义，并组合客户、产品、机构、币种和日期口径。                             |
| 📦 **数据消费工作台**       | 用同一份已发布存款余额，对比报表 / BI、TXT + FLAG 文件接口和 API 三种典型消费模式。                                               |
...
平台按照生产数仓的建设链路设计了 12 个渐进式章节，目前包含 49 节课程和 48 个深度交互实验：
...
- **第 04 章：SQL 与数据加工**（分层快照与任务加工契约）
- **第 05 章：调度系统**（DAG 依赖、定时触发、故障重试与分区补数）
- **第 06 章：数据质量**（规则校验、异常证据链与发布闸门）
- **第 07 章：数据血缘**（表级与字段级血缘追踪、变更影响面分析）
- **第 08 章：数据治理**（资产盘点、字段策略与生命周期管理）
- **第 09 章：湖仓架构**（数据湖的灵活性与数仓治理能力的结合）
- **第 10 章：数据服务**（报表 / BI、TXT + FLAG 文件接口与 API 消费模式）
