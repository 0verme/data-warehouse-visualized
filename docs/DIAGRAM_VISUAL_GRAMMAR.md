...
### 11.6 Phase 3 新增成员（增量登记）

§11.2–§11.5 保留 Phase 1 审计基线；Phase 3 起新增的 diagram 成员在实现 PR 中按同一列语义增量登记，不回写历史行。

|   # | Lesson / section · component                          | 主类型 + 辅助类型       | 教学问题                                                                     | N / C / L / Z / S / F / M                                                                                                                                                                                                                                                                                                        | R · 当前 token / 语义问题                                                                                        | Pilot |
| --: | ----------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ----- |
|  55 | `lifecycle-path-failure` · `LifecyclePathLab`（#137） | State + Flow / Evidence | 同一个任务为什么会在目标对象「不存在」和「已经存在」时走向两条生命周期路径？ | N=Day 1 / Day 2、目标对象、生命周期路径、日期写入单元、验证项；C=state transition（initialize / maintain）；L=目标对象状态→路径→阶段→结果；Z=运行前目标状态 / 当前业务日期写入单元；S=SUCCESS、FAILED、prepare、write、规则缺口 / 已单独验证；F=当前 Day 与 step reveal 的证据；M=无 timer，prev / next / reset 只切换确定性快照 | R2；T0+T1。状态由阶段文字与标签表达，颜色只作辅助；Evidence Panel 与 step reveal 同步，Step 1–2 不提前泄露根因。 | —     |
